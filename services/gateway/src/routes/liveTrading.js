...
class LiveTradingRoutes {
  ...
  async verifyOtp(request, reply) {
    try {
      ...
      const { onchainosVerifyOtp, onchainosWalletStatus, getWalletBalances } = this.okx;
      ...
      try {
        ...
        console.warn('[verifyOtp] 余额查询失败:', e.message);
      }
  ...
  async getWalletStatus(request, reply) {
    try {
      ...
      if (getWalletBalances) {
        ...
        } catch (e) { console.warn('[getWalletStatus] 余额查询失败:', e.message); }
      }
  ...
  async logoutWallet(request, reply) {
    try {
      ...
      await this.db.query(
        `UPDATE agentic_wallets SET status = 'logged_out', expires_at = NOW() WHERE user_id = $1`,
        [userId]
      );
      return reply.send({ code: 200, message: '已退出登录' });
    } catch (error) {
