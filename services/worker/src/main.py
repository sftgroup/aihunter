...
from src.price_refresh import PriceRefreshEngine
"""
...
class ChainWorker:
    ...
    async def connect(self):
        ...
        self.risk_engine = RiskEngine(self.db, self.redis, self.http)
        self.price_refresh = PriceRefreshEngine(self.db, self.redis, self.http)
        ...
        for chain in self.chains:
            ...
            if url:
                ...
                if self.lending_engine:
                    ...
                    if self.arb_engine:
                        self.arb_engine.add_chain(chain, url)
                    if self.risk_engine:
                        self.risk_engine.add_chain(chain, url)
