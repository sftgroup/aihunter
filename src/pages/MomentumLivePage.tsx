...
export default function MomentumLivePage() {
  ...
  const authorized = !!defaultWallet?.authorized;
      ...
      <span style={{ color: T.accentGreen, fontWeight: 700 }}>{String(newVal)}{s[paramKey] || ''}</span>
            ...
            <div style={cardBase}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                ...
                {wallets.length > 0 && (
                  <button onClick={() => { setShowAddWallet(true); setLoginStep('email'); }}
                    style={{ background: 'rgba(99,102,241,0.15)', border: '1px solid rgba(99,102,241,0.3)', borderRadius: 8, color: T.accent, fontSize: 11, padding: '5px 12px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}
                  ><Plus size={12} /> 添加地址</button>
                )}
              </div>
              ...
              {walletLoading ? (
                ...
                <div style={{ maxHeight: 220, overflowY: 'auto' }}>
                  {wallets.map(w => (
                    <div key={w.wallet_address}
                      ...
                      <p style={{ fontSize: 10, color: T.dark500, marginTop: 2, fontFamily: 'monospace', wordBreak: 'break-all' }}>
                        {w.wallet_address}
                      </p>
                    </div>
                  ))}
                ...
                <div style={{ textAlign: 'center', paddingTop: 8 }}>
                  {loginStep === 'email' && (
                    <div style={{ padding: '0 4px' }}>
                      ...
                      {loginError && <p style={{ fontSize: 11, color: T.accentRed, marginTop: 8 }}>{loginError}</p>}
                      <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                        <button onClick={handleCancelLogin} style={{ flex: 1, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, color: T.dark300, fontSize: 12, padding: '8px 0', cursor: 'pointer' }}>取消</button>
                        <button onClick={handleSendOtp} disabled={loginSending} style={{ flex: 1, background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', border: 'none', borderRadius: 10, color: 'white', fontSize: 12, fontWeight: 600, padding: '8px 0', cursor: loginSending ? 'default' : 'pointer', opacity: loginSending ? 0.6 : 1 }}>{loginSending ? '发送中...' : '发送验证码'}</button>
                      </div>
                    </div>
                  )}
                  {loginStep === 'otp' && (
                    <div style={{ padding: '0 4px' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                        <button onClick={handleCancelLogin} style={{ flex: 1, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, color: T.dark300, fontSize: 12, padding: '8px 0', cursor: 'pointer' }}>取消</button>
                        <button onClick={handleVerifyOtp} disabled={loginVerifying} style={{ flex: 1, background: 'linear-gradient(135deg, #10b981, #059669)', border: 'none', borderRadius: 10, color: 'white', fontSize: 12, fontWeight: 600, padding: '8px 0', cursor: loginVerifying ? 'default' : 'pointer', opacity: loginVerifying ? 0.6 : 1 }}>{loginVerifying ? '验证中...' : '验证并创建地址'}</button>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div style={{ textAlign: 'center', paddingTop: 16, paddingBottom: 20 }}>
                  ...
                  <p style={{ fontSize: 10, color: T.dark500, marginBottom: 16 }}>用你的邮箱创建独立 TEE 安全钱包</p>
                  <button onClick={() => { setShowAddWallet(true); setLoginStep('email'); }}
                    style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', border: 'none', color: 'white', borderRadius: 10, padding: '10px 28px', fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                    <Wallet size={14} /> 连接钱包</button>
                </div>
              )}
            </div>
